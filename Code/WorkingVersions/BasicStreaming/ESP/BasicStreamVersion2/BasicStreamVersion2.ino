...
uint8_t packetBuffer[chunkSize-1]; //where the chunks are put together in order won't include the byte that says chunk number
uint8_t chunkNumber;
...
const uint8_t frameReceived[3] = {16,0,0};
...
void loop() {
  ...
  switch(mode){
    case 0:  //IDLE
      IdleAnim();
      IdleLogic();
      OLEDUpdateTimer(); //Changed this so in this version it only updates in IDLE to save resources
      oledDisplay.clearDisplay();
      break;
    case 1: //STREAMING
      StreamLogic();
...
void StreamLogic(){
  ...
  for (int x = 0; x < PANEL_WIDTH; x++){
    for (int y = 0; y < PANEL_HEIGHT; y++){   
      while (packetSize <= 0){
        packetSize = udp.parsePacket(); //if no packet wait till one arrives
        udp.read(packetBuffer, 768);
        if (packetSize > 0){ //if a packet was received. Reset counters and exit loop
          byteCounter = 0;
        }
      }

      matrix->drawPixelRGB888(x,y,packetBuffer[byteCounter],packetBuffer[byteCounter+1],packetBuffer[byteCounter+2]);
      byteCounter += 3;
      //me adding 3 makes me nervous but this wouldn't even wokr if i couldn't do this anyway

      packetSize -= 3; //once this equals zero or under it will go back in the while loop
      
    }
  }

  //once it exits the nested loops it will go back around and do the same thing

  //This method will probably result in funky looking frames when even one packet gets lost. so we shall see how this goes.
  //more interested in how fast it can go.
}
