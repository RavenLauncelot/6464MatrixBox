...
const uint8_t frameReceived[1] = {16};
uint8_t frameFailed[ChunkPerFrame+1] = {17};  //amount fo chunks per frame plus the command byte. This will be edited 
...
void setup() {
  ...
  udp.beginPacket("192.168.0.64", PORT);
  udp.print("LED_MATRIX");
  udp.endPacket();
...
void StreamLogic(){
  ...
  //if timeout has happened. Request missing frames. If the time passsed since last packet was 20ms
    ...
    //If it equals zero it means it hasn't started sending packets yet
    if (timeLastPacket == 0){
      Serial.println("No packet sent yet");
    }      
